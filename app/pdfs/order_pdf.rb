class OrderPdf < Prawn::Document
  ...
  def order_number
    ...
    text "Заказчик: #{@order.user_name}"
    text "Номер телефона: #{@order.phone_number}"
  ...
  def line_item_rows
    [["Продукт", "Количество", "Цена за единицу", "Полная цена"]] +
        @order.line_items.map do |item|
          [item.product.product_name, item.quantity, price(item.product.price), price(item.product.price * item.quantity)]
        end
  end
